...
const { matchAnchors, selectActionItems } = require('../src/findings');
...
test('matches anchors to selected findings and drops weak or unknown ones', () => {
  const items = selectActionItems({
    requirements: [requirement('r-1', 'SuggestedByClover', 'High')],
    threats: [threat('t-1', 'SuggestedByClover', 'High')],
  });

  const placed = matchAnchors(items, [
  ...
  assert.equal(placed[0].anchor.startLine, 12);
});
