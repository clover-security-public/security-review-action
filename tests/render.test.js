...
test('links the title and footer to the review in Clover when a URL is known', () => {
  const { renderFindingComment } = require('../src/render');
  const cloverReviewUrl = 'https://app.cloversec.io/applications/app-1/reviews/rev-1';

  const summary = renderReviewComment({ cloverReviewUrl, requirements: [], summary: { summary: 'Summary.' }, threats: [] });

  assert.match(summary, /^## 🍀🛡️ \[Clover Security Review\]\(https:\/\/app\.cloversec\.io\/applications\/app-1\/reviews\/rev-1\)$/m);
  assert.match(summary, /\[Open the full review in Clover\]\(https:\/\/app\.cloversec\.io\/applications\/app-1\/reviews\/rev-1\)/);
  assert.doesNotMatch(summary, /github\.com\/clover-security-public/);

  const inline = renderFindingComment({
    cloverReviewUrl,
    finding: { id: 'thr-1', severity: 'Medium', threat: 'Token replay' },
    kind: 'Threat',
  });

  assert.match(inline, /^\*\*🍀🛡️ \[Clover Security Review\]\(https:\/\/app\.cloversec\.io\/applications\/app-1\/reviews\/rev-1\)\*\*$/m);

  const timeout = renderTimeoutComment({}, cloverReviewUrl);

  assert.match(timeout, /^## 🍀🛡️ \[Clover Security Review\]\(https:\/\/app\.cloversec\.io/m);
});

test('keeps the title as plain text when the tenant hides Clover links', () => {
  const summary = renderReviewComment({ cloverReviewUrl: null, requirements: [], summary: { summary: 'Summary.' }, threats: [] });

  assert.match(summary, /^## 🍀🛡️ Clover Security Review$/m);
  assert.match(summary, /_Generated by Clover Security\._/);
