...
const TITLE = 'Clover Security Review';

// Links to the review in the Clover web app; plain text when the tenant hides Clover links from developers.
function linked(text, cloverReviewUrl) {
  return cloverReviewUrl ? `[${text}](${cloverReviewUrl})` : text;
}

// Comments are posted by the workflow's token (usually github-actions[bot]), so the body itself
// carries the Clover identity.
function renderInlineCommentHeader(cloverReviewUrl) {
  return `**🍀🛡️ ${linked(TITLE, cloverReviewUrl)}**`;
}

function renderTitle(cloverReviewUrl) {
  return `## 🍀🛡️ ${linked(TITLE, cloverReviewUrl)}`;
}
...
function renderFindingComment({ cloverReviewUrl, finding, kind, location }) {
  const lines = [findingCommentMarker(finding.id), renderInlineCommentHeader(cloverReviewUrl), ''];
...
function renderReviewComment({ cloverReviewUrl, inlineFindingCount = 0, requirements, run, summary, threats }) {
  const summaryText = summary.summary ?? summary.shortSummary;

  const sections = [
    STICKY_COMMENT_MARKER,
    renderTitle(cloverReviewUrl),
    '',
  ];
  ...
  const generatedBy = cloverReviewUrl
    ? `_Generated by Clover Security · [Open the full review in Clover](${cloverReviewUrl})_`
    : '_Generated by Clover Security._';

  sections.push('---', ...renderRunFooter(run), '', generatedBy);

  return sections.filter((section) => section !== null).join('\n').replace(/\n{3,}/g, '\n\n');
}

function renderTimeoutComment(run, cloverReviewUrl) {
  return [
    STICKY_COMMENT_MARKER,
    renderTitle(cloverReviewUrl),
    '',
